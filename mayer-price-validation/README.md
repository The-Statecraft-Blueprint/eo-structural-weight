...
Validating against Mayer & Price answers a specific, falsifiable question: **does a rule-based structural reading of the text reconstruct expert judgment about significance, well enough to trust it as a signal on its own?** The answer so far is a qualified yes. AUC = 0.784 (95% CI 0.731–0.835): a randomly chosen significant order outscores a randomly chosen other order 78% of the time. Word count alone does almost as well (0.78), and the score adds to it rather than replacing it. The blind coder could also see each order's number, title and date, which a masked re-coding test is now checking. See `comparison-to-mayer-price.md` for where the measures diverge, and `../robustness/AUDIT.md` for the qualifications.
